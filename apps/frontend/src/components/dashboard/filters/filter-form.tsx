...
import { Checkbox, DateRangePicker, MultiCascader, SelectPicker } from 'rsuite';
...
const FilterForm: React.FC = () => {
  ...
  function buildStateTree(states: string[]): StateNode[] {
    return Object.values(
      states.reduce((acc, value) => {
        ...
        if (
          !stateNode.children.some(
            (child: StateNode) =>
              child.value === `${PIMMNumber}-${stateKey}-${stateValue}`
          )
        ) {
  ...
  const sortedStatesKey = useMemo(() => { 
    return JSON.stringify([...filters.states].sort());
  }, [filters.states]);

  const stateData = useMemo(
    () => buildStateTree(Array.from(filters.states)),
    [sortedStatesKey]
  );
  return (
    <>
      <DateRangePicker
        ...
        value={
          filters.initTime && filters.endTime
            ? [new Date(filters.initTime), new Date(filters.endTime)]
            : null
        }
      />
